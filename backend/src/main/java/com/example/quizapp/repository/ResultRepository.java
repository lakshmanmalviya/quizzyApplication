...
public interface ResultRepository extends JpaRepository<Result, Long> {
	...
	@Query("SELECT COUNT(r) FROM Result r WHERE r.user.id = :id")
	Long findTotalIncompleteQuizzesByUserId(@Param("id") Long id);

	@Query("SELECT r FROM Result r " + "WHERE (:userId IS NULL OR r.user.id = :userId) "
			+ "AND (:query IS NULL OR LOWER(r.quiz.title) LIKE LOWER(CONCAT('%', :query, '%'))) "
			...
			+ "AND (:timeLimit IS NULL OR r.quiz.timeLimit <= :timeLimit)")
