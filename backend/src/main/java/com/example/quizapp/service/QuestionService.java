...
public class QuestionService {
	...
	public UnifiedResponse<Long> delete(Long id) {
		if (questionRepository.existsById(id)) {
			...
			throwException(id);
		return commonHelper.returnUnifiedOK("Deleted", id);
	}

	@Transactional
	public UnifiedResponse<Question> update(Long id, QuestionRequest request) {
		Question question = getQuestionById(id);
		Question checkAlreadyWithThisName = questionRepository.findByTextAndQuizId(request.getText(),
				request.getQuizId());

		if (checkAlreadyWithThisName != null && checkAlreadyWithThisName.getId() != question.getId())
			throwException("Please change the name because the question with this name already exists");
	...
	public UnifiedResponse<PageResponse<Question>> findQuestionsByFilters(Boolean toggle, Long creatorId, Long quizId,
			String startDate, String endDate, String query, Boolean randomizeOptions, String questionType, String sort,
			Pageable pageable) {

		if (toggle == false && getUser().getRole().toString().equals("Educator"))
