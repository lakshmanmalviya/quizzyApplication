...
public class UserService {
	...
	public UnifiedResponse<User> updateUser(Long id, UpdateUserRequest request) {
		...
		return commonHelper.returnUnifiedOK("Updated successfully", userRepository.save(user));
	}

	public UnifiedResponse<Long> deleteUser(Long id) {
		User user = userRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("User not found with id: " + id));
		user.setIsDeleted(true);
		userRepository.save(user);
		return commonHelper.returnUnifiedOK("Deleted user", id);
	}
	...
	public UnifiedResponse<PageResponse<User>> findUsersByFilters(Role role, String query, Boolean isDeleted,
			String startDate, String endDate, String sort, Pageable pageable) {
		...
		Page<User> educators = userRepository.findUsersByFilters(roles, dates[0], dates[1], query, isDeleted, pageable);
