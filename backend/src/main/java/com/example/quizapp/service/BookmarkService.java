...
public class BookmarkService {
	...
	public UnifiedResponse<Bookmark> bookmarkQuiz(BookmarkRequest request) {
		if (bookmarkRepository.existsByUserIdAndQuizId(getUser().getId(), request.getQuizId())) {
			throw new ResourceAlreadyExistsException("Already bookmarked please bookmark other one");
		}
		Quiz quiz = quizService.findById(request.getQuizId());
		Bookmark bookmark = new Bookmark();
		bookmark.setQuiz(quiz);
		bookmark.setUser(getUser());
		return commonHelper.returnUnifiedOK("Bookmarked", bookmarkRepository.save(bookmark));
	}

	public UnifiedResponse<Long> deleteBookmarkById(Long id) {
		if (!bookmarkRepository.existsById(id)) {
			throwException(id);
		}
		bookmarkRepository.deleteById(id);
		return commonHelper.returnUnifiedOK("Removed", id);
	}
