...
import { deleteUserRequest, fetchUsersRequest, resetUserMessage } from '@/redux/slices/usersSlice';
...
const UsersTable: React.FC = () => {
    const { users, user, userLoading, userError, userMessage } = useAppSelector((state: RootState) => state.user);
    ...
    useEffect(() => {
        if (userMessage) {
            toast.success(userMessage, { position: 'top-center', });
            ...
            <Box className={styles.mainContainerOfTable}>
                ...
                <Box className={styles.tableBox}>
                    <TableContainer component={Paper} className={styles.tableContainer}>
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow className={userLoading ? `${styles.onReset} ${styles.rowStyle}` : ''}>
                                        ...
                                        <TableRow key={user.id}>
                                            ...
                                            >
                                                <TableCell component="th" scope="row" align="left">
                                                    {user.name.length <= 17 ? user.name : user.name.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
                                            ...
                                            >
                                                <TableCell align="left" style={{ color: 'gray' }}>
                                                    {user.email.length <= 17 ? user.email : user.email.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
                                            ...
                                            >
                                                <TableCell align="left">
                                                    {user.bio.length <= 17 ? user.bio : user.bio.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
                                            ...
                                            >
                                                <TableCell align="left">
                                                    {user.education.length <= 17 ? user.education : user.education.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
