...
import { createCategoryPopupReset, deleteCategoryRequest, fetchCategoriesRequest } from '@/redux/slices/categorySlice';
...
const CategoriesTable: React.FC = () => {
    const { categories, category, categoryLoading, categoryMessage, categoryError } = useAppSelector((state: RootState) => state.category);
    ...
    useEffect(() => {
        if (categoryMessage) {
            toast.success(categoryMessage, { position: 'top-center', autoClose: 500 });
    ...
    return (
        <div>
            <Box className={styles.mainContainerOfTable}>
                ...
                <Box className={styles.tableBox}>
                    <TableContainer component={Paper} className={styles.tableContainer}>
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow className={categoryLoading ? `${styles.onReset} ${styles.rowStyle}` : ''}>
                                        ...
                                        <TableRow key={category.id}>
                                            ...
                                            >
                                                <TableCell component="th" scope="row" align="left">
                                                    {category.name.length <= 17 ? category.name : category.name.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
                                            ...
                                            >
                                                <TableCell align="left">
                                                    {category.description.length <= 17 ? category.description : category.description.substring(0, 17) + "..."}
                                                </TableCell>
                                            </Tooltip>
            ...
            {openConfirmBox && <CategoryDeleteConfirmModal onClose={handleCloseConfirmBox}
