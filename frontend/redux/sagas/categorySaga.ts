...
function* handleCreateCategory(action: ReturnType<typeof createCategoryRequest>) {
    try {
        const response: UnifiedResponse<Category> = yield call(apiCall, getBaseUrl(), 'POST', getAuthenticatedHeader(), action.payload);
        yield put(createCategorySuccess({ data: response.data, msg: response.msg }));
    } catch (error) {
...
function* handleDeleteCategory(action: ReturnType<typeof deleteCategoryRequest>) {
    try {
        ...
        yield put(deleteCategorySuccess({ id: response.data, msg: response.msg }));
    } catch (error) {
...
function* handleUpdateCategory(action: ReturnType<typeof updateCategoryRequest>) {
    try {
        const response: UnifiedResponse<Category> = yield call(apiCall, `${getBaseUrl()}/${action.payload.id}`, 'PUT', getAuthenticatedHeader(), action.payload);
        yield put(updateCategorySuccess({ data: response.data, msg: response.msg }));
    } catch (error) {
