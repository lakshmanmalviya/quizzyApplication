...
function* handleDeleteBookmark(action: ReturnType<typeof deleteBookmarkRequest>) {
    try {
        const response: UnifiedResponse<number> = yield call(apiCall, `${getBookmarkUrl()}/${action.payload}`, 'DELETE', getAuthenticatedHeader());
        yield put(deleteBookmarkSuccess({ id: response.data, msg: response.msg }));
    } catch (error) {
