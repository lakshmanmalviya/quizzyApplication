...
export const checkIsBase64Icon = (str: string | undefined): string => {
    const base64Regex = /^data:image\/[a-zA-Z]+;base64,/;
    if (base64Regex.test(str ?? ''))
        return str ?? '/addPic.png';
    return '/addPic.png';
};
...
export const giveTime = (time: number): string => {
    const given_seconds: number = time;
    const dateObj: Date = new Date(given_seconds * 1000);
    const hours: number = dateObj.getUTCHours();
    const minutes: number = dateObj.getUTCMinutes();
    const seconds: number = dateObj.getSeconds();
    const timeString: string = hours.toString().padStart(2, '0') + ':' +
...
export const quizSortingOptions = [
    ...
    { id: 'pass:asc', name: 'Pass%  ASC' },
    { id: 'pass:desc', name: 'Pass%  DESC' },
]
